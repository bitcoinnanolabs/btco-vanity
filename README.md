...
Generate a BTCO address with a prefix of your choice.
...
To install `btco-vanity` from crates.io:

```
cargo install btco-vanity
```

To install `btco-vanity` from source:
...
For a list of `btco-vanity` options, use `btco-vanity --help`.

## Seed Generation

By default, `btco-vanity` generates private keys instead of seeds.
...
```
btco-vanity --threads 1 --no-progress --limit 0 --simple-output btco_1 | cut -d' ' -f1 | xxd -r -p | dieharder -a -g stdin_input_raw
```
...
To be even more careful, you can modify btco-vanity's parameters.
