...
fn main() {
    let args = clap::App::new("btco-vanity")
        .version(env!("CARGO_PKG_VERSION"))
        .author("Lee Bousfield <ljbousfield@gmail.com>")
        .about("Generate BTCO cryptocurrency addresses with a given prefix")
        .arg(
    ...
    if let Some(mut prefix) = args.value_of("prefix") {
        ...
        if prefix_chars.next().is_some() {
            ...
            eprintln!(
                "Only the first 60 characters of your prefix (not including btco_) will be used."
            );
            eprintln!("");
        }
    }
